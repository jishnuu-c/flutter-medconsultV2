...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  Widget _headlineBlock() {
    return const Column(
      ...
      children: [
        Text(
          'Advanced Healthcare & Clinical Consultations',
        ...
        SizedBox(height: 12),
        Text(
          'Streamlined medical consultations, digital EMR prescriptions, and multi-clinic scheduling in one unified platform.',
          textAlign: TextAlign.center,
          style: TextStyle(
            fontSize: 12.3,
            height: 1.6,
  ...
  // .auth-card mobile: max-width 460, radius 20, padding 32px 24px
  Widget _authCard() {
    return Container(
      ...
      child: Column(
        children: [
          // .auth-card::before — 4px gradient top bar
          ClipRRect(
  ...
  Future<void> loginWithGoogle() async {
    // Same as Angular: window.location.href = `${environment.apiUrl}/oauth2/authorization/google`
    // Backend does the Google OAuth dance, then redirects to /oauth-success?token=...
    final url = Uri.parse('$kBaseUrl/oauth2/authorization/google');
    final launched = await launchUrl(
      url,
      webOnlyWindowName: '_self', // full-page nav on web, matches Angular
    );
    if (!launched && mounted) {
      setState(() => _errorMessage = 'Could not open Google sign-in.');
    }
  }
}
